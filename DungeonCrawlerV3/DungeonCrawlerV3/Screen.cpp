...
std::vector<const Image*> Screen::_imagesToRender;
...
void Screen::AddImages(std::initializer_list<const Image*> images) {
    _bufferUpdateLock.lock();
...
void Screen::RemoveImages(std::initializer_list<const Image*> images) {
    _bufferUpdateLock.lock();
    for (auto& image : images) {
    ...
    for (const Image* image : _imagesToRender) {
        ...
        for (int row = 0; row < image->GetImage().size(); ++row) {
            ...
            for (int character = 0; character < image->GetImage()[row].size(); ++character) {
                if (startingCoords.first + character + ORIGIN_HORIZONTAL_OFFSET >= SCREEN_WIDTH) break; //stop processing line if it's off the screen
